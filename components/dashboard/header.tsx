...
export function DashboardHeader({
  ...
  const getPageTitle = () => {
    ...
    if (pathname?.includes("/products"))
      return finalUserType === "buyer" ? "Market" : "Products";
  ...
  const userInitials =
    avatarFallback || userNameValue.substring(0, 2).toUpperCase();
  ...
  return (
    <header className="fixed top-0 right-0 left-0 md:left-[240px] flex h-14 items-center gap-4 border-b bg-gradient-to-r from-amber-50 to-amber-100 shadow-md px-4 lg:px-6 z-10">
      <Button
        variant="outline"
        size="icon"
        className="md:hidden bg-amber-100 border-amber-200 hover:bg-amber-200"
      ...
      <div className="w-full flex-1">
        ...
        {pathname?.includes("/messages") && (
          <p className="text-xs text-muted-foreground ml-7">
            Connect with your{" "}
            {finalUserType === "buyer" ? "suppliers" : "customers"}
          </p>
        )}
      </div>
      <div className="flex items-center gap-2">
        <Button
          variant="outline"
          size="icon"
          className="bg-amber-100 border-amber-200 hover:bg-amber-200"
        >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              className="gap-1 bg-amber-100 border-amber-200 hover:bg-amber-200"
            >
            ...
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                ...
                <p className="text-xs leading-none text-muted-foreground">
                  {userRole || (userType === "buyer" ? "Buyer" : "Farmer")}
                </p>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
